...
// Handle insert new student info (if not exists)
if (isset($_POST['insert_info'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO students (id, fullname, email, phone, department_id) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("isssi", $user_id, $fullname, $email, $phone, $department_id);
    $stmt->execute();
    $stmt->close();

    $msg = "Your profile has been inserted.";
...
<head>
    <meta charset="UTF-8">
    <title>Student Dashboard - Campus Event System</title>
    ...
    <style>
        ...
        .sidebar h4 { color: #fff; margin-bottom: 26px; }
        .sidebar a { color: #fff; text-decoration: none; display: block; margin-bottom: 16px; font-size: 1.06rem; font-weight: 500;}
        ...
        .profile-info h5 { font-size: 1.22rem; font-weight: 700; color: #23649b;}
...
<div class="row gx-0">
    ...
    <div class="col-md-3 sidebar">
        <h4><i class="bi bi-person-badge me-2"></i>Student Panel</h4>
        <a href="student_dashboard.php" class="active"><i class="bi bi-house me-2"></i>Dashboard</a>
        <a href="#events"><i class="bi bi-calendar3 me-2"></i>Events</a>
        <a href="#profile"><i class="bi bi-person-lines-fill me-2"></i>My Profile</a>
        <a href="logout.php"><i class="bi bi-box-arrow-left me-2"></i>Logout</a>
    </div>
    <!-- Main Content -->
    <div class="col-md-9 main-content">
        <h2 class="mb-4"><i class="bi bi-house"></i> Student Dashboard</h2>
        <?php if (isset($msg)): ?><div class="msg mb-3"><?= htmlspecialchars($msg) ?></div><?php endif; ?>

        <!-- Student Profile Card or Insert Info Form -->
        <div id="profile">
            <?php if ($student): ?>
                <div class="profile-card">
                    <img src="<?= isset($student['photo']) && $student['photo'] ? htmlspecialchars($student['photo']) : 'https://ui-avatars.com/api/?name='.urlencode($student['fullname']).'&background=2471A3&color=fff&size=120' ?>"
                         alt="Student Photo" class="profile-img">
            ...
            <?php else: ?>
                <div class="insert-section mb-4">
                    <h5 class="mb-3"><i class="bi bi-person-plus"></i> Insert Your Information</h5>
                    <form method="post">
                        <div class="mb-3">
                        ...
                        <button type="submit" name="insert_info" class="btn btn-success"><i class="bi bi-person-plus"></i> Insert Information</button>
                    </form>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="update-section mb-4">
            <h5 class="mb-3"><i class="bi bi-pencil-square"></i> Update Your Information</h5>
            <form method="post">
                <div class="mb-3">
                    <label class="form-label">Full Name</label>
                ...
                <button type="submit" name="update_info" class="btn btn-primary"><i class="bi bi-check-circle"></i> Update Info</button>
            </form>
        </div>
        <?php endif; ?>

        <!-- Events Section -->
        <h4 class="mb-3" id="events"><i class="bi bi-calendar3"></i> Campus Events</h4>
        <div class="row">
                    ...
                    <div class="event-card mb-4">
                        ...
                        <div class="event-details">
                            ...
                            <div class="event-actions">
                                ...
                                <?php if ($student): ?>
                                <form method="post">
                                    <input type="hidden" name="fullname" value="<?= htmlspecialchars($student['fullname']) ?>">
                                    <input type="hidden" name="email" value="<?= htmlspecialchars($student['email']) ?>">
                                    <input type="hidden" name="phone" value="<?= htmlspecialchars($student['phone']) ?>">
                                    <input type="hidden" name="department_id" value="<?= htmlspecialchars($student['department_id']) ?>">
                                    <button type="submit" name="update_info" class="btn btn-primary btn-sm">
